...
install.packages(c("readxl", "dplyr", "lubridate", "ggplot2", "factoextra",
                   "cluster", "writexl", "GGally", "plotly"))
library(readxl)
library(dplyr)
library(lubridate)
library(ggplot2)
...
# Create box plots for each RFM variable
...
# Step 10: 3D Visualization of Clusters using plotly

# 3D Scatter Plot using original RFM variables
fig <- plot_ly(
  data = rfm_clustered,
  x = ~Recency,
  y = ~Frequency,
  z = ~Monetary,
  color = ~Cluster,
  colors = c('#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'),  # Adjust colors as needed
  type = 'scatter3d',
  mode = 'markers',
  marker = list(size = 3, opacity = 0.8)
) %>%
  layout(
    title = '3D Scatter Plot of RFM Variables by Cluster',
    scene = list(
