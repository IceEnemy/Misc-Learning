...
plot(hc_result, main = "Dendrogram of Hierarchical Clustering", xlab = "", sub = "", cex = 0.9)

k <- 3

rfm_clustered <- rfm_data_transformed %>%
  mutate(Cluster = as.factor(cutree(hc_result, k = k)))


# 2D visualization
fviz_cluster(list(data = rfm_scaled, cluster = cutree(hc_result, k = k)),
             palette = "jco",
             geom = "point",
             ellipse.type = "convex",
             ggtheme = theme_minimal()) +
  labs(title = "Customer Segments Visualization using Hierarchical Clustering")
...
# Silhouette Score
silhouette_score <- silhouette(cutree(hc_result, k), dist_matrix)
